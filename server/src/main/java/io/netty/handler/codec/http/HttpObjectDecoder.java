 ...
 * 对解码器进行了部分重写，原来内容具体可以查看 {@link io.netty.handler.codec.http.HttpObjectDecoder}
 * 只重写了读取结果实现中文识别，readHeaders 方法中的请求头自动进行了中文转换
 * 搜索 “此处为修改部分” 即可找到修改部分
 * @since 1.0.0
 */
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
    ...
    protected HttpObjectDecoder(
        ...
        lineParser = new HttpObjectDecoder.LineParser(seq, maxInitialLineLength);
        headerParser = new HttpObjectDecoder.HeaderParser(seq, maxHeaderSize);
        ...
        switch (currentState) {
            ...
            case READ_INITIAL: try {
                ...
                if (initialLine.length < 3) {
                    ...
                    currentState = HttpObjectDecoder.State.SKIP_CONTROL_CHARS;
                    return;
                }

                message = createMessage(initialLine);
                currentState = HttpObjectDecoder.State.READ_HEADER;
                // fall-through
            } catch (Exception e) {
            ...
            case READ_HEADER: try {
                HttpObjectDecoder.State nextState = readHeaders(buffer);
                if (nextState == null) {
                    return;
                }
            ...
            case READ_CHUNK_SIZE: try {
                ...
                if (chunkSize == 0) {
                    currentState = HttpObjectDecoder.State.READ_CHUNK_FOOTER;
                    return;
                }
                currentState = HttpObjectDecoder.State.READ_CHUNKED_CONTENT;
                // fall-through
            } catch (Exception e) {
            ...
            case READ_CHUNKED_CONTENT: {
                ...
                currentState = HttpObjectDecoder.State.READ_CHUNK_DELIMITER;
                // fall-through
            }
            case READ_CHUNK_DELIMITER: {
                ...
                while (wIdx > rIdx) {
                    ...
                    if (next == HttpConstants.LF) {
                        currentState = HttpObjectDecoder.State.READ_CHUNK_SIZE;
                        break;
                    }
                }
    ...
    protected void decodeLast(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (message != null) {
            ...
            if (currentState == HttpObjectDecoder.State.READ_VARIABLE_LENGTH_CONTENT && !in.isReadable() && !chunked) {
                // End of connection.
                out.add(LastHttpContent.EMPTY_LAST_CONTENT);
                resetNow();
                return;
            ...
            if (currentState == HttpObjectDecoder.State.READ_HEADER) {
    ...
    private void resetNow() {
        ...
        if (!isDecodingRequest()) {
            ...
            if (res != null && isSwitchingToNonHttp1Protocol(res)) {
                currentState = HttpObjectDecoder.State.UPGRADED;
                return;
            }
        }
    ...
    private HttpMessage invalidMessage(ByteBuf in, Exception cause) {
        currentState = HttpObjectDecoder.State.BAD_MESSAGE;
    ...
    private void splitHeader(AppendableCharSequence sb) {
        ...
        if (valueStart == length) {
            ...
            value = new String(sb.subStringUnsafe(valueStart, valueEnd).getBytes(StandardCharsets.ISO_8859_1),StandardCharsets.UTF_8);
        }
    }
